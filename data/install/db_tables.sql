...
CREATE TABLE `#@__users` (
  ...
  `dt_last_login` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '上次登录时间',
...
CREATE TABLE `#@__members` (
  ...
  `description` varchar(512) NOT NULL DEFAULT '' COMMENT '描述',
  `dt_last_rerank` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '上次更新成长度时间',
  `dt_created` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`member_id`),
...
CREATE TABLE `#@__poll_member_logs` (
  ...
  `ip_last_modified` bigint(20) NOT NULL DEFAULT '0' COMMENT '上次投票IP',
  PRIMARY KEY (`log_id`),
