...
CREATE TABLE `tr_generator_field_types` (
  ...
  KEY `sort` (`sort`)
) ENGINE=InnoDB AUTO_INCREMENT=19 DEFAULT CHARSET=utf8 COMMENT='表单字段类型表';
...
CREATE TABLE `tr_generator_field_validators` (
  ...
  KEY `sort` (`sort`)
) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=utf8 COMMENT='表单字段验证表';
...
) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=utf8 COMMENT='表单字段表';
...
INSERT INTO `tr_generator_fields` VALUES ('5', 'category', '0', 'n', 'n', '类型，app：应用、mod：模块、ctrl：控制器、act：行动', '1', '1', '1', '5', '类型', '', 'n', 'n', 'y', '0', 'y', '0', 'y', '0', 'n', '0');
